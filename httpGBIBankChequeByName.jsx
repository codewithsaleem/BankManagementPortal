...
class ChequeByName extends Component {
    state = {
        cheques: { items: [], },
    }
    ...
    async fetchData() {
        ...
        let response = null;
        if (searchString) {
            response = await http.get(`/getChequeByName/${user.name}?${searchString}`);
        }
        else {
            response = await http.get(`/getChequeByName/${user.name}?page=1`);
        ...
        return (
            <div className="container mt-3">
                <div className="jumbotron">
                    <h3 className="text-center text-secondary">All Cheque Details</h3>
